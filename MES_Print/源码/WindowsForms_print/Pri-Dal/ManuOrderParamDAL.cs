...
namespace ManuOrder.Param.DAL
{
    class ManuOrderParamDAL
    {
            ...
            using (SqlCommand command = conn1.CreateCommand())
            {
                ...
                while (dr.Read())
                {
                    list.Add(new Gps_ManuOrderParam()
                    {
                        ...
                        Remark4 = dr.IsDBNull(28) ? "" : dr.GetString(28),
        ...
        //根据制单号更新remark5
        public int UpdateRemark5DAL(string ZhiDanNum,string remark4, string remark5)
        {
            SqlConnection conn1 = new SqlConnection(conStr);
            conn1.Open();
            using (SqlCommand command = conn1.CreateCommand())
            {
                command.CommandText = "UPDATE Gps_ManuOrderParam SET Remark4='"+ remark4 + "', Remark5 ='" + remark5 + "' WHERE ZhiDan='" + ZhiDanNum + "'";
                return command.ExecuteNonQuery();
            }
        }
