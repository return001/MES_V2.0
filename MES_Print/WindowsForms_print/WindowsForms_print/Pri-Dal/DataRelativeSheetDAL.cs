...
namespace DataRelative.Param.DAL
{
    ...
    class DataRelativeSheetDAL
    {
        ...
        public string SelectIccidBySimAndImeiDAL(string SIM,string IMEI)
        {
            SqlConnection conn1 = new SqlConnection(conStr);
            conn1.Open();
            string Iccid;
            SqlCommand command = conn1.CreateCommand();
            command.CommandText = "SELECT * FROM dbo.DataRelativeSheet WHERE (IMEI1 = '"+IMEI+"' AND IMEI3 ='" + SIM + "')";
            SqlDataReader dr = command.ExecuteReader();
            if (dr.Read())
            {
        ...
        public string SelectIMEIBySnOrIMEI2DAL(string IMEI2OrSn, string IMEI2Value)
        {
            SqlConnection conn1 = new SqlConnection(conStr);
            conn1.Open();
            string IMEI;
            SqlCommand command = conn1.CreateCommand();
            command.CommandText = "SELECT * FROM dbo.DataRelativeSheet WHERE "+IMEI2OrSn+" = '" + IMEI2Value + "'";
            SqlDataReader dr = command.ExecuteReader();
            if (dr.Read())
            {
