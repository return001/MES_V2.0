...
public class ExcelHelper {
	...
	/**
	 * 根据实体类填写指定行，不带样式 需要在实体类的每个属性中注解@Excel，col属性代表列号，从0开始，head属性表示该字段的表头描述
	 * 
	...
	/**
	 * 根据实体类填写指定行，使用Body样式 需要在实体类的每个属性中注解@Excel，col属性代表列号，从0开始，head属性表示该字段的表头描述
	 * 
	 * @throws IllegalAccessException
	 * @throws IllegalArgumentException
	 */
	...
	/**
	 * 根据实体类填写指定行，使用自定义样式 需要在实体类的每个属性中注解@Excel，col属性代表列号，从0开始，head属性表示该字段的表头描述
	 * 
	 * @throws IllegalAccessException
	 * @throws IllegalArgumentException
	 */
	...
	/**@author HCJ
	 * 根据集合从某一行开始填写表格，使用自定义样式
	 * @param records 集合
	 * @param title 标题
	 * @param head 列名称
	 * @date 2019年6月5日 上午10:26:05
	 */
	public void fillStringList(List<String> records, String title, String head) {
		fillStringList(headStyle, bodyStyle, records, 1, head);
		Sheet sheet = workbook.getSheetAt(currentSheetNum);
		if (sheet.getRow(1) == null) {
			return;
		}
	...
	public void fillStringList(CellStyle headStyle, CellStyle bodyStyle, List<String> records, int startRowNum, String head) {
		if (records != null) {
			for (int i = 1; i <= records.size(); i++) {
				String record = records.get(i - 1);
				// 如果是第一行则填写表头
				if (i == startRowNum) {
					set(i, 0, head, headStyle);
				}
				try {
					set(i + 1, 0, record == null ? "" : record, bodyStyle);
				} catch (IllegalArgumentException e2) {
					logger.error("调用ExcelHelper.fill()中field.get()方法时出错");
					e2.printStackTrace();
				}
