...
public class SQL {
	...
	public final static String SELECT_LINE_PROCESSGROUP = "SELECT line.id, line_no AS 'lineNo', line_name AS lineName, line_remark AS lineRemark, line_director AS lineDirector, line_engineer AS lineEngineer, line_qc AS lineQc, confirm_time_from AS confirmTimeFrom, confirm_time_to as confirmTimeTo, process_group AS processGroup, process_group.group_name AS groupName, sop_factory.abbreviation as factoryName, sop_workshop.workshop_name AS workshopName,sop_factory.id as factoryId,sop_workshop.id as workshopId, a.Name AS directorName, b.Name AS engineerName, c.Name AS qcName FROM line, process_group, sop_workshop, sop_factory, LUserAccount a, LUserAccount b, LUserAccount c WHERE line.process_group = process_group.id AND line.factory_id = sop_factory.id AND line.workshop_id = sop_workshop.id AND a.Id = line.line_director AND b.Id = line.line_engineer AND c.Id = line_qc ";
