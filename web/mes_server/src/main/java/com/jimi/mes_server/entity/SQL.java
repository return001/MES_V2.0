...
public class SQL {
	...
	public final static String SELECT_MODELCAPACITY = "SELECT model_capacity.id,soft_model as softModel,customer_model as customerModel,model_capacity.process_group as processGroup,process_people_quantity as processPeopleQuantity,capacity,remark, process_group.group_name as groupName,rhythm,customer_name as customerName,customer_number as customerNumber,transfer_line_time as transferLineTime,sop_factory.abbreviation FROM model_capacity, process_group,sop_factory WHERE model_capacity.process_group = process_group.id AND sop_factory.id = process_group.factory ";
	...
	public final static String SELECT_PLAN_INFORMATION = "SELECT orders.ZhiDan,orders.soft_model,scheduling_quantity,line_computer.ip FROM scheduling_plan LEFT JOIN orders ON orders.id = scheduling_plan.orders LEFT JOIN line_computer ON line_computer.line = scheduling_plan.line WHERE scheduling_plan.id = ? ";
