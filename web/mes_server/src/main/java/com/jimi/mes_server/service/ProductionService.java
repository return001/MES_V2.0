...
public class ProductionService {
	...
	public boolean addLine(String lineNo, String lineName, String lineRemark, Integer lineDirector, Integer processGroup) {
	...
	public boolean editLine(Integer id, String lineNo, String lineName, String lineRemark, Integer lineDirector,Integer processGroup) {
		Line line =  Line.dao.findById(id);
		if (line==null) {
		...
		int indexOfOrderItem = 2;
		 try {
			...
			for (OrderItem orderItem : orderItems) {
				if (!StringUtils.isAnyBlank(orderItem.getZhidan(),orderItem.getSoftModel())) {
					Orders temp =Orders.dao.findFirst(SQL.SELECT_ORDER_BY_ZHIDAN, orderItem.getZhidan());
					if (temp!=null) {
						resultString = "导入失败，请检查表格第" + indexOfOrderItem + "行的料号或数量是否填写了准确信息！";
						return resultString;
					}
				}
				indexOfOrderItem++;
