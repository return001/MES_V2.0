...
public class RoleService {
	...
	private static final String SELECT_AUTHORITY_BY_ROLE = "SELECT authority.* FROM authority INNER JOIN role_authority ON role_authority.auhtoriry = authority.id WHERE role_authority.role = ? ";
	...
	private static final String SELECT_ROLE_BY_NO_AND_DEPARTMENT = "SELECT * FROM role WEHRE no = ? AND department = ?";
	
	private static final String SELECT_ROLE_BY_NAME_DEPARTMENT  = "SELECT * FROM role WEHRE name = ? AND department = ?";

	private static final String BASE_SELECT_ROLE_INFO = "SELECT sop_factory.id AS companyId, sop_factory.abbreviation AS companyName, department.id AS departmentId, department.name AS departmentName, role.id AS roleId, role.name AS roleName, role.no AS roleNo FROM sop_factory INNER JOIN department ON department.company = sop_factory.id INNER JOIN role ON department.id = role.department WHERE role.is_typcial = 1 ";
	...
	public List<AuthorityVO> getAuthority(Integer role) {
		
		List<Authority> authorities = Authority.dao.find(SELECT_AUTHORITY_BY_ROLE, role);
		
		return AuthorityVO.fillList(authorities);
	}
