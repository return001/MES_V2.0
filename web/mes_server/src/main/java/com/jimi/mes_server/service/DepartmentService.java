...
public class DepartmentService {
	...
	private static final String SELECT_DEPARTMENT_BY_COMPNAY_AND_NAME = "SELECT department.* FROM department INNER JOIN sop_factory ON department.compnay = sop_factory.id WHERE department.name = ? AND department.company = ? AND department.is_enabled = 1";;
	
	private static final String SELECT_ROLE_BY_DEPARTMENT = "SELECT * FROM role WEHRE department = ?";
	
	private static final String BASE_SELECT_DEPARTMENT_INFO = "SELECT sop_factory.id AS companyId, sop_factory.abbreviation AS companyName, department.id AS departmentId, department.name AS departmentName, department.[no] AS departmentNo FROM sop_factory INNER JOIN department ON department.company = sop_factory.id INNER JOIN role ON department.id = role.department WHERE department.is_enabled = 1 ";
	...
	public void add(Integer company, String no, String name) {
		...
		department.setNo(no).setName(name).setCompany(company).setIsEnabled(true).update();
	}
