...
import com.jimi.mes_server.model.UserActionLog;
...
public class UserActionLogInterceptor implements Interceptor {
	...
	public void intercept(Invocation invocation) {
		UserLog log = invocation.getMethod().getAnnotation(UserLog.class);
		...
		if (log != null) {
			...
			actionLog.setAddress(ip);
			actionLog.setTime(new Date());
			LUserAccountVO loginedUser = TokenBox.get(controller.getPara("#TOKEN#"), UserController.SESSION_KEY_LOGIN_USER);
			if (loginedUser != null) {
				actionLog.setUid(loginedUser.getName());
			}
			// 执行
			try {
				...
				// 插入
				actionLog.setConsumeTime((int) (System.currentTimeMillis() - beforeTime));
				actionLog.save();
			} catch (Exception e) {
