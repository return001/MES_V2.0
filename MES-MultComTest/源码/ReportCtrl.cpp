...
{
	WNDCLASS wndclass;
	AFX_MANAGE_STATE(AfxGetStaticModuleState());//AfxGetModuleState
...
BEGIN_MESSAGE_MAP(CReportCtrl, CWnd)
	...
	ON_COMMAND_RANGE(ID_EIDT_DELETE, ID_EIDT_DELETE, OnMenuDelete)
	ON_COMMAND(ID_EIDT_NEW, OnMenuNew)
...
void CReportCtrl::OnRButtonDown(UINT nFlags, CPoint point)
{
	...
	if(rvhti.iItem != RVI_INVALID)
	{
		...
		if(!(nFlags&(MK_CONTROL|MK_SHIFT)))
		{
			...
			CMenu menu;
			VERIFY( menu.LoadMenu( IDR_MENULIST ) );		
...
void CReportCtrl::OnMenuDelete(UINT x)
{
	DeleteItem(m_iSelectRow);
}

void CReportCtrl::OnMenuNew()
{
	RVITEM rvi;
	InsertItem(m_iSelectRow,"");
	rvi.iItem = m_iSelectRow;
	rvi.nMask = RVIM_CHECK;//²åÈëcheckbox
	rvi.iSubItem = 1;
	rvi.iCheck = 0;
	SetItem(&rvi);

	rvi.nMask = RVIM_TEXT;//²åÈëedittext
	rvi.iSubItem = 2;
	rvi.lpszText = "";
	SetItem(&rvi);
...
{
	WNDCLASS wndcls;
	//AFX_MANAGE_STATE(AfxGetModuleState());//AfxGetModuleState  AfxGetStaticModuleState
	HINSTANCE hInst = NULL;//AfxGetInstanceHandle();//Handle to the instance of the application that created the class.
