...
BOOL CUserSetting::UpdateUserToDb(CString Name, CString Password, CString Limits, BOOL ChangePw, CString NewPassword)
{
	if (1)//数据库地址不为空，就上传数据
	{
		...
		if (Barcode_Check == TRUE)
		{
			...
			{
				strSQL_Write_Barcode = "UPDATE LUserAccount SET " + Field + " = '" + Limits + "',Password = '" + NewPassword + "' WHERE Name='" + Name + "' AND Password='" + Password + "'";
			}
			else
			{
				strSQL_Write_Barcode = "UPDATE LUserAccount SET " + Field + " = '" + Limits + "' WHERE Name='" + Name + "' AND Password='" + Password + "'";
			}
...
void CUserSetting::OnBnClickedButton6()
{
	...
	DeleteUserFromDb(UserName, Password);
}


BOOL CUserSetting::DeleteUserFromDb(CString Name, CString Password)
{
	if (1)//数据库地址不为空，就上传数据
	{
		myado.OpenSheet("select * from LUserAccount WHERE Name='" + Name + "' AND Password='" + Password + "'");
