...
CActiveSock::CActiveSock(HANDLE StopEvent)
{
	...
	LastError = 0;
	RecvInitiated = false;
	RecvTimeoutSeconds = 150;			// Default timeout is 1 seconds, encourages callers to set it
	SendTimeoutSeconds = 150;			// Default timeout is 1 seconds, encourages callers to set it
...
CActiveSock::~CActiveSock()
{
	...
	WSACleanup();
	RELEASE_SOCKET(ActualSocket);
}

/////////////////////////////////////////////////////////////////////////////
// CActiveSock member functions
bool CActiveSock::Connect(LPCTSTR HostName, USHORT PortNumber, CString IPAddr)
{
	int iResult;
	...
	ActualSocket = socket(AF_INET, SOCK_STREAM, 0);//IPPROTO_TCP-->最后一个参数
	...
	serAddr.sin_addr.S_un.S_addr = inet_addr((LPCTSTR)IPAddr);
	if (connect(ActualSocket, (sockaddr *)&serAddr, sizeof(serAddr)) == SOCKET_ERROR)
	{
	...
	////////////////////////////
	//WIN10不支持
	//iResult = setsockopt(ActualSocket, SOL_SOCKET,0x7010 /*SO_UPDATE_CONNECT_CONTEXT*/, NULL, 0);
	...
	if (WSAIoctl(ActualSocket, SIO_KEEPALIVE_VALS, &sKA_Settings,sizeof(sKA_Settings), &sReturned, sizeof(sReturned), &dwBytes,NULL, NULL) != 0)
	{
		LastError = WSAGetLastError() ;
